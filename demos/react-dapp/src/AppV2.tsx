...
export default function V2App() {
  ...
  const storageKey = 'hedera-wc-demos-saved-state-v2';
  ...
  useEffect(() => {
    ...
    if (!selectedSigner && dAppConnector?.signers.length > 0) {
      setSelectedSigner(dAppConnector.signers[0])
    }
  ...
  const handleInitAppKit = async () => {
     ...
     */
    const signClient = walletConnectProvider.client;
    const universalProvider = await UniversalProvider.init({
      projectId,
      metadata,
      client: signClient as unknown as UniversalProviderOpts['client'],
    })
    ...
    appKit.subscribeEvents((args) => {
      console.log('subscribeEvents')
      console.log({ args })
    })
        appKit.subscribeAccount((args) => {
          console.log('subscribeAccount')
          console.log({ args })
        })
    setDAppConnector(walletConnectProvider.dappConnector)
    setAppKit(appKit)
    saveData()
  }
  ...
  const handleConnect = async () => {
    try {
      ...
      if (!appKit) throw new Error('appKit is required')
        ...
        <section>
          <div>
            ...
            <fieldset>
              ...
              <button disabled={!appKit || isLoading} onClick={() => handleConnect()}>
                Connect Wallet
              </button>
